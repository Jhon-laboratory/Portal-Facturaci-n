<?php
/**
 * Guarda datos de despacho - VERSIÓN CORREGIDA (actualiza cabecera)
 */
...
class GuardadoDespachoRapido {
    ...
    public function ejecutar($input) {
        ...
        try {
            ...
            // Procesar factura (AHORA ACTUALIZA LA CABECERA)
    ...
    if (!$input['factura_id']) {
        ...
        $sql = "INSERT INTO " . TABLA_FACTURAS . " 
                (cliente_id, cliente_codigo, usuario_id, usuario_nombre, 
                 despacho_archivo, despacho_completado, fecha_creacion) 
                OUTPUT INSERTED.id
                VALUES (?, ?, ?, ?, ?, 1, GETDATE())";
        ...
        error_log("✅ NUEVA factura despacho: ID $factura_id");
        
    } else {
        // ============================================
        // CASO 2: FACTURA EXISTENTE (UPDATE)
        // ============================================
        $sql_delete = "DELETE FROM " . TABLA_DESPACHO . " WHERE factura_id = ?";
        $stmt_delete = $this->conn->prepare($sql_delete);
        $stmt_delete->execute([$input['factura_id']]);
        
        $sql_update = "UPDATE " . TABLA_FACTURAS . " 
                       SET despacho_archivo = ?, 
                           despacho_completado = 1
                       WHERE id = ?";
    ...
    private function insertarPorLotes($factura_id, $datos) {
        ...
        foreach ($lotes as $indice => $lote) {
            ...
            try {
                ...
                foreach ($lote as $row) {
                    $fecha = $this->procesarFecha($row['ADDDATE'] ?? '');
                ...
                $sql = "INSERT INTO " . TABLA_DESPACHO . " 
                        (factura_id, ORDERKEY, SKU, STORERKEY, EXTERNORDERKEY, 
                         UNIDADES, CAJAS, PALLETS, STATUS, ADDDATE, TYPE) 
...
try {
    ...
    $guardado = new GuardadoDespachoRapido();
